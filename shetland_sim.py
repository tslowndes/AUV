...
AUV.waypoints = [[waypoints.iloc[i].lon, waypoints.iloc[i].lat, -1*waypoints.iloc[i].depth] for i in range(waypoints.shape[0])]
print(AUV.waypoints)
for elps_time in range(config.run_time):
    AUV.go(config, elps_time)
    update_progress(elps_time, config.run_time)
write_proof(AUV,config,'../Results/data/Shetland_Comparison/shetland_sim.csv')